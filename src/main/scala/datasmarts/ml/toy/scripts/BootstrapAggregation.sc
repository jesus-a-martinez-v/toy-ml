...
def subsample(dataset: Dataset, ratio: Double = 1.0, randomizer: Random) = {
  val nSample = math.round(dataset.length * ratio).toInt

  randomizer.shuffle(dataset)
...
def bagging(train: Dataset, test: Dataset, parameters: Parameters) = {
  ...
  val trees = (1 to numberOfTrees).toList.map(_ => buildTree(subsample(train, sampleSize, random), maxDepth, minSize))
